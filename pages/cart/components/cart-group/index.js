...
Component({
  ...
  properties: {
    storeGoods: {
      type: Array,
      observer(storeGoods) {
        if (!Array.isArray(storeGoods)) return;

        for (const store of storeGoods) {
          if (!store.promotionGoodsList || !Array.isArray(store.promotionGoodsList)) continue;
          for (const activity of store.promotionGoodsList) {
            if (!activity.goodsPromotionList || !Array.isArray(activity.goodsPromotionList)) continue;
          ...
          if (!store.shortageGoodsList || !Array.isArray(store.shortageGoodsList)) continue;
    ...
    invalidGoodItems: {
      ...
      observer(invalidGoodItems) {
        if (!Array.isArray(invalidGoodItems)) return;
